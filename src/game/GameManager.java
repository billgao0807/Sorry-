...
import sorryclient.SoundLibrary;
...
public class GameManager {
	...
	public void drawCard() {
		...
		SoundLibrary.playSound("audios/draw_card.wav");
		canDraw = false;
		////System.out.println("Disabled " + GameHelpers.getNameFromColor(livePlayers[currentPlaying].getColor()) + " to draw card");
		//		OutMail.send(MessageConstants.BROADCAST, MessageConstants.START_TIMER);
		startTimer();
...
//		//System.out.println("GM: Simulation click of " + playerName + x + " " + y);
	...
	public void endTurn() {
		...
		if(allHome) {
			...
			if(currentPlaying == mainPlayer) {
				SoundLibrary.playSound("audios/victory.wav");
			} else {
				SoundLibrary.playSound("audios/lose.wav");
			}
		...
		//		//System.out.println("GM: End turn for " + GameHelpers.getNameFromColor(livePlayers[currentPlaying].getColor()));
		...
		if(card.getType() != CardDeck.TWO) {
			...
			if(currentPlaying == mainPlayer) {
				...
				//				//System.out.println("GM: Broadcasting end turn to increase currentPlaying");
	...
	//	private void moveTile(Color color) {
	//		if(mDestTile == null) return ;
	//		mGamePanel.stopTimer();
	//		
	...
	//			if(toCheck == mDestTile) {
	//				movePawnsForward(color);
	//			}
	//		}
	//		else if( cardType == CardDeck.FOUR) {
	//			Tile toCheck = travel(mSelectedTile, color, -cardValue);
	//			if(toCheck == mDestTile) {
	//				movePawnsBackward(color);
	//			}
	//		}
	//	}

	//	private void movePawnsForward(Color color){
	//		Tile tempTile = mSelectedTile;
	//		while(tempTile != mDestTile){
	...
	//	private void movePawnsBackward(Color color) {
	//		
	//	}

	public void sendEndTurn() {
		if (currentPlaying == mainPlayer ){
			OutMail.send(MessageConstants.BROADCAST, MessageConstants.TIME_OUT);
		}
	}
	...
	public void nextPlayer() {
		...
		//		//System.out.println("GM: Current playing after + 1 " + GameHelpers.getNameFromColor(livePlayers[currentPlaying].getColor())+" and main player is " + GameHelpers.getNameFromColor(livePlayers[mainPlayer].getColor()));
		...
		if (currentPlaying == mainPlayer) {
			SoundLibrary.playSound("audios/your_turn.wav");
		}
	...
	public synchronized boolean tileClicked(Tile tile, Player player) {
		//Check basic impossibilities
		if(card == null) return false;
		if(tile == null) return false;
		if(!livePlayers[currentPlaying].equals(player)) return false;

		//System.out.println("GM: " + GameHelpers.getNameFromColor(player.getColor()) + " clicked " + tile.getX() + " " + tile.getY());
		...
		if(card.getType() == CardDeck.SORRY) {
			...
			if(!livePlayers[currentPlaying].isActive()) {
				...
				//System.out.println("Sorry tile not occupied");
				return false;
			}
		}
		...
		if(swapTile != null) {
			...
			if(swapTile == null) {
				...
				//System.out.println("Swap tile null");
				return false;
			}
		}
		...
		if(!start && !owned) {
			//System.out.println("Tile !start ! owned");
			return false;
		}
		...
		if(pawn == null) {
			//System.out.println("No pawn to work with");
			return false;
		}
		...
		if(card.getType() == CardDeck.SEVEN && sevenRemainder != -1) {
			...
			if(endTile == null) {
				//System.out.println("Seven remainin tile travel return null");
				return false;
			}
			...
			// first remove other player's pawn if at destination tile and clear any other players' pawns if on slide
			if(endTile.isOccupied()) players[GameHelpers.getIndexFromColor(endTile.getPawnColor())].returnPawn(endTile.getPawn());
			//						if(beforeTile != endTile) slideRemove(beforeTile, pawn.getColor());

			final Tile destTile = endTile; 
			new Thread(new Runnable(){
				@Override
				public void run() {
					Color color = player.getColor();
					Tile tempTile = tile;
					//								//System.out.println("GM: move from " + tile.getX() + " " + tile.getY() + " to " + destTile.getX() + " " + destTile.getY());

					while(tempTile != destTile){
						try {
							Thread.sleep(250);
							//										////System.out.println("GM: Try sleep 250");
						...
						if(!tempTile.isHome()){
							if (!tempTile.isOccupied()){
								tempTile.setPawn(pawn);
							}
						} else {
							//							tempTile.setPawn(pawn);
							pawn.setCurrentTile(tempTile);
						}
			...
			if(endTile == null) {
				//System.out.println("Regular tile travel return null");
				return false;
			}
			Tile beforeTile = endTile;
			endTile = slide(endTile,pawn);
			if(endTile.isOccupiedByColor(pawn.getColor())) {
				if(endTile.getPawn() != pawn) {
					//System.out.println("Regular tile cannot land on own pawn");
					return false;
				}
			}
			...
			Tile endTile = travel(tile, pawn, -CardDeck.getValue(card));
			if(endTile == null) {
				//System.out.println("Four travel return null");
				return false;
			}
			Tile beforeTile = endTile;
			endTile = slide(endTile,pawn);
			if(endTile.isOccupiedByColor(pawn.getColor())) {
				if(endTile.getPawn() != pawn) {
					//System.out.println("Four no land on own pawn");
					return false;
				}
			}
			//			if(endTile.isOccupied()) players[GameHelpers.getIndexFromColor(endTile.getPawnColor())].returnPawn(endTile.getPawn());
			...
			new Thread(new Runnable(){
				...
				public void run() {
					...
					for (int i = 0; i < 4; ++i){
						...
						tempTile.removePawn();
						if (tempTile.getNext(pawn.getColor()).isSafeZone()) {
							tempTile = tempTile.getNext(pawn.getColor());
						} else {
...
//					if (tempTile != destTile) {
//						tempTile.removePawn();
//						if(!destTile.isHome()){
//							destTile.setPawn(pawn);
//						} else pawn.setCurrentTile(destTile);
//					}
				}			
			}).start();
			//						tile.removePawn();
			...
			if(options == 0) {
				//System.out.println("Seven no valid move");
				return false;
			}
			else {
				...
				if(endTile == null) {
					//System.out.println("Seven travel return null");
					return false;
				}
				Tile beforeTile = endTile;
				endTile = slide(endTile,pawn);
				if(endTile.isOccupiedByColor(pawn.getColor())) {
					if(endTile.getPawn() != pawn) {
						//System.out.println("Seven no land on own pawn");
						return false;
					}
				}
				//				if(endTile.isOccupied()) players[GameHelpers.getIndexFromColor(endTile.getPawnColor())].returnPawn(endTile.getPawn());
			...
			if(!livePlayers[currentPlaying].isActive()){
				...
				if(canMoveForward) {
					...
					final Tile destTile = endTileForward; 
					new Thread(new Runnable(){
						@Override
						public void run() {
							Color color = player.getColor();
					...
					//					if(beforeTileBackward != endTileBackward) slideRemove(beforeTileBackward, pawn.getColor());
					//
					//					final Tile destTile = endTileBackward; 
					//					new Thread(new Runnable(){
					//						@Override
					//						public void run() {
					//							Color color = player.getColor();
...
//				start = start.getPrevious();
				if (start.getNext(pawn.getColor()).isSafeZone()) {
	...
	public Tile travel(Tile start, Color color, int numSpaces) {
		...
		for(int i = 0; i < numSpaces; i++) {
			if(backwards) {
				if (start.getNext(color).isSafeZone()) {
...
//				start = start.getPrevious();
	...
	public boolean highlightPath(Tile destination, Color color) {
		...
		if(
			...
			if(distance > 7 || distance <= 0) {
				//System.out.println("Seven travel too far or negative");
				return false;
			}
			//sevenSplit = distance;
	...
	private void highlightTilesBackward(Tile from, Tile to, Color color) {
		...
		while(tempTile != to){
			tempTile.highlighted = true;
			if (tempTile.getNext(color).isSafeZone()) {
				tempTile = tempTile.getNext(color);
			} else {
				tempTile = tempTile.getPrevious();
			}
//			tempTile = tempTile.getPrevious();
	...
	public void deactivatePlayer(String name) {
		if (name != null && players != null && livePlayers!=null) {
